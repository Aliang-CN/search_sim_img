...
class BatchEmbGen(object):
    ...
    def pipeline(self):
        ...
        index = 0
        while True:
            batch_img_id, batch_img_url,  last_index, flag = self.download_engine.get_batch_img_id(index=index, limit=5)
            if flag == 1:
                break
            index = last_index
            for i in range(5):
        ...
        while True:
            for i in range(5):
                try:
                    ...
                    batch_sim_vec_list = [[id for id, dis in zip(id_list, distance_list) if dis < 250.0 and dis > 20.0]
                                          for id_list, distance_list in zip(search_result.id_array,
                                                                            search_result.distance_array)]
...
def main():
    # 生成embedding
    beg = BatchEmbGen(mode='test')
    beg.pipeline()

    # 计算相似度
    bsc = BatchSimCal(mode='test')
    bsc.pipeline()
